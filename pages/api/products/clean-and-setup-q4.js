...
// CLEAN STORE - Keep only Q4 winners
export default async function handler(req, res) {
  await connectDB()

  if (req.method === 'POST') {
    try {
      console.log('🧹 Cleaning store - Keeping only Q4 winners...')

      // Delete ALL existing products
      await Product.deleteMany({})
      console.log('✅ Cleared all existing products')

      // Add ONLY the 3 Q4 winners
      const q4Winners = [
        {
          ...
          sku: 'LED-STRIP-5M-Q4',
          stock: 1000,
          tags: ['led', 'smart-home', 'rgb', 'alexa', 'home-decor', 'gaming', 'christmas', 'trending', 'q4-winner'],
          specifications: {
            'Length': '5 meter',
        ...
        },
        {
          ...
          sku: 'PHONE-RING-Q4',
          stock: 2000,
          tags: ['phone', 'ring', 'grip', 'magsafe', 'must-have', 'trending', 'gadget', 'protection', 'q4-winner'],
          specifications: {
        ...
        },
        {
          ...
          sku: 'CABLE-ORG-Q4',
          stock: 800,
          tags: ['cables', 'organization', 'home-office', 'gaming', 'must-have', 'trending', 'modern', 'q4-winner'],
          specifications: {
      ...
      const products = await Product.insertMany(q4Winners)
      console.log(`✅ Store cleaned - Only ${products.length} Q4 winners now`)

      res.status(200).json({
        success: true,
        message: `Store cleaned! Only ${products.length} Q4 winning products`,
