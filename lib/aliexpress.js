...
export async function exchangeCodeForToken(code) {
  ...
  try {
    console.log('🔍 Debug - Token exchange parameters:', {
      ...
      code: code ? '✅ Aanwezig' : '❌ Ontbreekt',
      appKey: appKey ? `✅ ${appKey.substring(0, 3)}...` : '❌ Ontbreekt',
      appSecret: appSecret ? '✅ Aanwezig' : '❌ Ontbreekt',
      callbackUrl: callbackUrl
    })

    // AliExpress OAuth token exchange - probeer eerst client_id (OAuth 2.0 standaard)
    // Als dat niet werkt, probeer app_key
    // Parameters moeten in request body (niet als query parameters)
    const params = new URLSearchParams({
      grant_type: 'authorization_code',
      code,
      client_id: appKey, // OAuth 2.0 standaard parameter
      client_secret: appSecret, // OAuth 2.0 standaard parameter
      redirect_uri: callbackUrl
    })

    console.log('🔍 Debug - Sending token exchange request to:', ALIEXPRESS_OAUTH_URL)
    console.log('🔍 Debug - Request parameters (without secrets):', {
      grant_type: 'authorization_code',
      code: code ? '✅ Aanwezig' : '❌ Ontbreekt',
      client_id: appKey ? `✅ ${appKey.substring(0, 3)}...` : '❌ Ontbreekt',
      redirect_uri: callbackUrl
    })
    console.log('🔍 Debug - Full client_id value:', appKey || 'MISSING')
    console.log('🔍 Debug - App status should be Online/Production now')

    const response = await axios.post(ALIEXPRESS_OAUTH_URL, params.toString(), {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      }
    })

    console.log('🔍 Debug - AliExpress OAuth response:', {
      status: response.status,
      hasData: !!response.data,
      dataKeys: response.data ? Object.keys(response.data) : [],
      fullData: JSON.stringify(response.data).substring(0, 500) // Eerste 500 chars voor debugging
    })

    if (response.data.error) {
      console.error('❌ AliExpress OAuth error response:', response.data)
    ...
    console.log('🔍 Debug - Parsed token data:', {
    ...
    if (!tokenData.accessToken || !tokenData.refreshToken) {
      ...
      throw new Error('AliExpress response bevat geen access_token of refresh_token. Response: ' + JSON.stringify(response.data))
    }
